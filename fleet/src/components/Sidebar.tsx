  ...
  return (
    <aside style={{ position:'fixed', left:0, top:0, height:'100%', width:'240px', background:'#0f172a', borderRight:'1px solid rgba(255,255,255,0.06)', display:'flex', flexDirection:'column', zIndex:50 }}>
      ...
      <nav style={{ flex:1, padding:'0.75rem', overflowY:'auto' }}>
        ...
        {navItems.map(item => {
          if ((item as any).supervisorOnly && user.role !== 'SUPERVISOR') return null
      ...
      <div style={{ padding:'0.75rem', borderTop:'1px solid rgba(255,255,255,0.06)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'10px', background:'rgba(255,255,255,0.03)', borderRadius:'10px', marginBottom:'6px' }}>
          ...
          <div style={{ minWidth:0 }}>
            <p style={{ fontSize:'12px', fontWeight:600, color:'#f8fafc', margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{user.name || user.email}</p>
            <span style={{ fontSize:'10px', padding:'1px 6px', borderRadius:'4px', background:'rgba(37,99,235,0.2)', color:'#93c5fd' }}>{ROLE_LABELS[user.role] || user.role}</span>
          </div>
        </div>
